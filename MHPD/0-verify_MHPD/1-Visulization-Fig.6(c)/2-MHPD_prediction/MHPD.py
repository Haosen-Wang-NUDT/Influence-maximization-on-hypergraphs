...
def MHPD(hyper_matrix, seeds, k, beta, model):
    """
    输入：
    ...
    infect_matrix = cal_infect_matrix(model, hyper_matrix, beta)
    ...
    MHPD_list = np.array(nodes_prob).sum(axis = 1)
        
    return MHPD_list[-1], MHPD_list
...
# path = '../0-hypergraph_datasets/Algebra.txt'
# dl = dataloader(path)
# dl.dataload()
...
# beta = 0.01
# model = 'CP'
# _, prob_sum = MHPD(df_hyper_matrix.values, [4, 5], k, beta, model)
# print('\n', prob_sum)
